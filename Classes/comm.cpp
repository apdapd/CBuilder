...
ECommError::ECommError(void)
   : Error(BAD_SERIAL_PORT),
     Errno(0)
{
}
     ...
     Errno(GetLastError())
{
   switch ( Error )
   {
      case BAD_BAUD_RATE: m_ErrorStr =    "ошибка установки бит рэйта";break;
      case BAD_STOP_BITS: m_ErrorStr =    "ошибка установки стоп битов";break;
      case BAD_PARITY: m_ErrorStr =       "ошибка установки чётности";break;
      case BAD_BYTESIZE: m_ErrorStr =     "ошибка установки размера бита";break;
      case PORT_ALREADY_OPEN: m_ErrorStr ="ошибка повторного открытия ком порта";break;
      case PORT_NOT_OPEN: m_ErrorStr =    "попытка доступа к не открытому ком порту";break;
      case OPEN_ERROR: m_ErrorStr =       "ошибка открытия ком порта";break;
      case WRITE_ERROR: m_ErrorStr =      "ошибка записи в ком порт";break;
      case READ_ERROR: m_ErrorStr =       "ошибка чтения из ком порта";break;
      case CLOSE_ERROR: m_ErrorStr =      "ошибка закрытия ком порта";break;
      case PURGECOMM: m_ErrorStr =        "ошибка очистки ком порта";break;
      case FLUSHFILEBUFFERS: m_ErrorStr = "ошибка сброса буферов ком порта";break;
      case GETCOMMSTATE: m_ErrorStr =     "ошибка получения статуса ком порта";break;
      case SETCOMMSTATE: m_ErrorStr =     "ошибка установки статуса ком порта";break;
      case SETUPCOMM: m_ErrorStr =        "ошибка функции SetupComm";break;
      case SETCOMMTIMEOUTS: m_ErrorStr =  "ошибка учтановки таймаутов";break;
      case CLEARCOMMERROR: m_ErrorStr =   "ошибка очистки ошибки ком порта";break;
   }
}
...
void TCommPort::OpenCommPort(void)
{
	...
	String Sss = String (m_CommPort.c_str());
	m_hCom = CreateFile(/*(LPTSTR)m_CommPort.c_str()*/Sss.t_str(),
...
void TCommPort::SetCommPort(const AnsiString port)
{
...
AnsiString TCommPort::GetCommPort(void)
{
    return m_CommPort;
}
...
void TCommPort::SetCommTimeouts ( COMMTIMEOUTS  nm_TimeOuts )
{
   m_TimeOuts = nm_TimeOuts;
}
